...
if __name__ == '__main__':
    ...
    if not pathlib.Path(BASE_DIR).joinpath('scripts', 'md.sample').exists():
        pathlib.Path(BASE_DIR).joinpath('scripts', 'md.sample').write_text(encoding='utf-8',data='')
    else:
        pass
    Post.objects.create(
    ...
    for post in Post.objects.all()[:20]:
        ...
        for _ in range(random.randrange(3, 15)):
            Comment.objects.create(
                ...
                email=fake.email(),
                url=fake.paragraph(),
                created_time=fake.date_time_between(start_date=delta_in_days,
                                                    end_date="now",
                                                    tzinfo=timezone.get_current_timezone()),
                                                    post=post,
                )
